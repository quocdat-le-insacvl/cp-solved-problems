...
void solve() {
    ...
    vector<float> A(N);
    rep(i, 0, N) {
       float floatValue = static_cast<float>(B[i]);
       A[i] = floatValue;
    }
    if (N == 4) {
        print((A[2]+A[3])/2 - (A[0]+A[1]) / 2);
    } else if (N == 5) {
        print(max(
                    -((A[0]+A[1]+A[2]) / 3 - (A[3] + A[4]) / 2), 
                    -((A[0]+A[1]) / 2 - (A[3] + A[4]+A[2]) / 3 ))
             );
    }else {
        int size = sz(A);
        //dbg(A);
        //dbg((A[size-1] + A[size-2])/2, (A[0] +A[1]) / 2);
        print(
              (A[size-1] + A[size-2])/2 -  (A[0] +A[1]) / 2
             );
    }
