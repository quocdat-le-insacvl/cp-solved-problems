...
void solve() {
    ...
    repi(N) {
        ...
        if (i > 0 && i < M) {
            ...
            prefix_sum[i] = A[i] + prefix_sum[i - 1] - A[i - M];
        }
    }
    vll final_sum(N);
    vll ans;
    ...
    repi(N) {
        ...
        if (i < M) {
            final_sum[i] = (i + 1) * A[i] + final_sum[i - 1];
        } else {
            final_sum[i] = M * A[i] + final_sum[i - 1] - prefix_sum[i - 1];
            ans.pb(final_sum[i]);
        }
        if (i == M - 1)
            ans.pb(final_sum[i]);
...
//    print(prefix_sum);
//    print(final_sum);
//    print(*max_element(final_sum.begin() + M - 1, final_sum.end()));
