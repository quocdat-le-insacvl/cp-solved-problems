...
//        print(mpos);
    ...
    void bfs_monster() {
        ...
        while (!dq.empty()) {
            // pop
            auto POP = dq.back();
            int i = POP.x.x, j = POP.x.y, cnt = POP.y;
            ...
            for (int k = 0; k < 4; k++) {
                ...
                int nj = j + dj[k];
                if (is_valid(ni, nj)) {
                    if ((g[ni][nj] == '.' || g[ni][nj] == 'B' || g[ni][nj] == 'A') && !visited[ni][nj]) {
                        visited[ni][nj] = true;
...
//            print(visited);
            ...
            if (g[i][j] == 'B' || (g[i][j] == 'A' && ( i == 0 || j == 0 || i == N-1 || j == M-1))) {
                print("YES");
                string paths;
                int ni = i, nj = j;
                while (ni != si || nj != sj) {
