...
void solve() {
    ...
    int N; read(N);
    vi A(N); read(A);
    vt<vt<array<ll, 2>>> dp(N, vt<array<ll, 2>>(N, {MIN_LL, MIN_LL}));
    auto res = [&] (auto&& res, int i,int j, bool turn=1) {
        if (dp[i][j][turn] != MIN_LL) {
            return dp[i][j][turn];
        }
        if (i > j) return 0ll;
        if (i == j) {
        ...
        dp[i+1][j][!turn] = res(res, i+1, j, !turn);
        dp[i][j-1][!turn] = res(res, i, j-1, !turn);
        if (turn)
            dp[i][j][turn] =max(A[i] + dp[i+1][j][!turn], A[j] + dp[i][j-1][!turn]);
        else 
            dp[i][j][turn] = min(dp[i+1][j][!turn], dp[i][j-1][!turn]);
        return dp[i][j][turn];

    };
    ll ans = res(res, 0, N-1);
    print(ans);
}
