...
public:
    ...
    void dfs_distance(int node, int parent, ll pre_dis) {
        if (node != 0)
            distance[node] = pre_dis + (N - subtree_size[node]) - (subtree_size[node]);
        EACH(child, children[node]) {
            if (child == parent) continue;
            dfs_distance(child, node, distance[node]);
        }
    }
...
//        print(depth);
        ...
        ll dis_root = 0;
        EACH(x, depth) dis_root += x;

        distance[0] = dis_root;
        dfs_distance(0, -1, dis_root);
        print(distance);
